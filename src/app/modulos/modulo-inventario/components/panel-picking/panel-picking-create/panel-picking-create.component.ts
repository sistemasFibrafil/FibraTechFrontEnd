...
export class PanelPickingCreateComponent implements OnInit, OnDestroy {
  ...
  modeloSolicitudList                           : ISolicitudTraslado[] = [];
  ...
  private loadDocumentList(): void {
    ...
    if (objType.value === '1250000001') {
      this.loadSolicitudOpenList();
    } else {
      //this.loadOrdenVentaOpenList();
    }
  }

  private loadSolicitudOpenList(): void {
    this.isDisplay = true;
    this.solicitudTrasladoService.getListOpen()
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data: ISolicitudTraslado[]) => {
          this.isDisplay = false;
          this.modeloSolicitudList = data;
          this.documentoList = this.modeloSolicitudList.map(item => ({
            label: item.docNum.toString(),
            value: item.docEntry
          }));
        },
        error: (e) => {
          this.documentoList = [];
          this.utilService.handleErrorSingle(e, 'loadSolicitudOpenList', () => { this.isDisplay = false; }, this.swaCustomService);
        }
      });
  }
  ...
  private loadOrdenVentaOpenList(): void {
    ...
    // this.documentoLecturaSapService.getListDocumentoPendienteByObjTypeAndCardCode()
    //   .pipe(takeUntil(this.destroy$))
    //   .subscribe({
    ...
    //     error: (e) => {
    //       this.documentoList = [];
    //       this.utilService.handleErrorSingle(e, 'loadOrdenVentaOpenList', () => { this.isDisplay = false; }, this.swaCustomService);
    //     }
    //   });
  }
  ...
  onChangeDocumento(): void {
    ...
    const solicitudSelected = this.modeloSolicitudList.find(item => item.docEntry === documento?.value);

    if (solicitudSelected) {
      const defaultSelectItem = this.almacenList.find(item => item.value === solicitudSelected.filler);
  ...
  private save(): void {
    ...
    const modeloToSave: PickingCreateModel = {
  ...
  private loadModalData(): void {
    ...
    const { u_CodeBar } = this.modeloFormModal.value;
    const value: any = {
