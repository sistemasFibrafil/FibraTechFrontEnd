...
export class PanelOrdenVentaEditComponent implements OnInit, OnDestroy {
  ...
  modeloLinesEliminar                           : IOrdenVenta1Query[] = [];
  ...
  private buildBulkMenuOptions(): void {
    this.menuOptions = [
      ...
      { value: '3',  label: 'Fact. reserva',      icon: 'pi pi-cart-plus', command: () => this.onClickToCopyReserveInvoice() },
    ];
  }
  ...
  private setFormValues(value: IOrdersQuery): void {
    ...
    this.modeloLines = (value.lines || []).map(linea => this.utilService.mapLine(linea, status));
  ...
  private onToSave() {
    ...
    this.ordersService.setUpdate(modeloToSave)
    .pipe(
      takeUntil(this.destroy$),
  ...
  //#region <<< 22. OBSERVABLES / WATCHERS >>>
